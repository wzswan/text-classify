...
class loadText(object):
    ...
    def __iter__(self):
        for file in os.listdir(self.par_path):
            file_abpath = os.path.join(self.par_path, file)
            if os.path.isfile(file_abpath):
                this_file = open(file_abpath, 'rb')
                content = this_file.read().decode('utf8')
                yield file_abpath,content
                this_file.close()


def convert_doc_to_wordlist(str_doc,cut_all):
    sent_list = str_doc.split('\n')
    sent_list = map(rm_char, sent_list) # delete some useless thing
    word_2dlist = [rm_tokens(jieba.cut(part,cut_all=cut_all)) for part in sent_list] # cut word_2dlist
    word_list = sum(word_2dlist,[])
    return word_list
def rm_tokens(words): #delete some function word and number
    words_list = list(words)
    stop_words = get_stop_words()
    for i in range (words_list.__len__())[::-1]:
        if words_list[i] in stop_words: # delete function words_list
            words_list.pop(i)
...
if __name__=='__main__':
    ...
    path_files = '/home/wzswan/Downloads/tool/DataSet/newweekdata/140609'
    ...
    ##==========================================================
    ### the first step , mapping all the documents,creat dictionay, and delete frequency low words
    # if the position no dictionary, so recreate a new one, if exists, skip it
    ...
    ##==========================================================
    #### the second step, start to change documents into tfidf
    if not os.path.exists(path_tmp_tfidf):
        print('===not find tf===')
        # if there is no tfidf, create one, if exists, skip it
        if not dictionary: # if skip the first step,then to read the dictionary from the specific position
        ...
        # take tfidf middle results stored
        ...
        for catg in catgs:
            corpora.MmCorpus.serialize('{f}{s}{c}.mm'.format(f=path_tmp_tfidf,s=os.sep,c=catg),
                                        ...
                                        id2word = dictionary)
            print('catg{c} has been transformend into tfidf vector'.format(c=catg))
    ...
    ###==============================================================
    ## new files classify with statistics
    if not dictionary:
        dictionary = corpora.Dictionary.load(path_dictionary)
    if not lsi_model:
    ...
    for file in files:
        ...
        if t not in catg_list:
            catg_list.append(t)
    #demo_doc = open(path_files,'rb')
    #files = loadFiles(path_files)
    demo_docs = loadText(path_files)
    counter1 = 0
    counter2 = 0
    for i, msg in enumerate(demo_docs):
        if i%n == 0:
            file_abspath   = msg[0]
            file           = msg[1]
            word_text = convert_doc_to_wordlist(file, cut_all=False)
            demo_bow = dictionary.doc2bow(word_text)
            tfidf_model = models.TfidfModel(dictionary=dictionary)
            ...
            if ('{x}'.format(x=catg_list[x[0]])) == "财经":
               counter1 += 1
            elif('{x}'.format(x=catg_list[x[0]])) == "股票":
               counter2 += 1
            else:
